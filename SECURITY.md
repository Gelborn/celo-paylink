...
- the exact PayLink path and query string, if safe to share, so URL parsing issues can be reproduced without relying on screenshots
...
- transaction hashes, PayLink receipt URLs, and matching Celo explorer transaction links, if applicable and safe to share
