...
Open the copied PayLink receipt URL in a signed-out browser once before sharing so reviewers see the same amount, token, reference, and handle without payer wallet state.
After a live payment, refresh the public profile once so reviewers can see the same amount, token, and reference in the latest PayLink contract event.
