...
After a live payment, refresh the public profile once so reviewers can see the same amount, token, and reference in the latest PayLink contract event on that profile.
...
- After a browser-wallet fallback network switch, re-check that PayLink still shows the same amount, token, reference, and recipient handle before sending.
