...
MiniPay PayLink is a mobile-first no-custody public payment profile for freelancers, creators, and solo merchants on Celo.

Create a public, no-custody payment profile, add your handle, avatar, bio, and preferred token, and receive Celo stablecoin payments through public profile URLs like:
...
3. Get paid directly to your wallet in Celo stablecoins with a shareable, reopenable PayLink receipt URL for client-ready proof.
...
- Share reopenable PayLink receipt URLs with matching Celo explorer transaction links for the same Celo transaction
...
For walletless local review, keep the placeholder `PRIVATE_KEY` from `.env.example`; replace it only before running deploy, verify, or seed scripts.
...
Use that printed network origin for local QR code, public profile, or localized PayLink prefilled payment request URL tests on the phone.
...
Open each recopied phone URL once before recording so stale localhost or prior LAN origins do not appear in reviewer footage.
...
For Brazilian Portuguese screenshots, use the in-app language switcher or set the browser language to Portuguese (Brazil) before capturing both preview pages.
...
For live demos, copy the PayLink receipt URL from the receipt page address bar and the matching Celo explorer transaction link after wallet confirmation so the `tx` value and handle match the real payment.
...
Open the copied PayLink receipt URL in a signed-out or private browser window once before sharing so reviewers see the same amount, token, reference, handle, and profile return path without payer wallet state.
...
- Use a low-value live payment for review on the selected Celo network so wallet confirmations and receipts stay verifiable without risking more funds than needed.
...
- Payers do not need a PayLink account; they only need a compatible Celo wallet to review the request and send the payment.
- Any browser wallet that can switch to the Celo network, approve ERC-20 allowances, and submit PayLink transactions can test the same profile and payment flows.
...
Run these locally from a clean worktree before publishing public changes or recording final demo footage:
...
The GitHub Actions CI workflow is manual-only; trigger the manual workflow for the submitted commit after the final push if you need a hosted verification record reviewers can match to the submitted commit SHA.

## Avatar URLs

- PayLink accepts remote avatar image URLs for this proof of concept.
- Avatar URLs must be `https://`.
- Use a direct image file URL that returns an image content type, such as `.jpg`, `.png`, or `.webp`, rather than a profile page or gallery link so preview fetchers can load it directly.
...
These token addresses come from the linked Celo token docs. Before a mainnet deployment, re-check them against the latest Celo token docs before copying production environment values.
...
- `robots.ts` and page metadata keep dashboard routes, PayLink receipt paths, and preview URLs out of indexing while PayLink receipt URLs still open directly for sharing.
...
See the launch checklist, demo script, and pitch outline:
