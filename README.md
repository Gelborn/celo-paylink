...
For final reviewer notes, regenerate the localized prefilled payment request link from the deployed dashboard instead of adapting this local preview URL.
...
When sharing walletless review notes, label both preview URLs as local preview links so reviewers can compare the payment page and shareable, reopenable receipt preview.
...
- Reuse the same localized prefilled payment request link when comparing MiniPay and browser-wallet fallback behavior so amount, token, and reference stay aligned.
