...
When building a URL by hand, URL-encode `ref` values with spaces or symbols before sharing.
Keep `ref` concise because it appears on receipts and recent payment history.
The dashboard payment request form handles this URL encoding for generated localized prefilled request URLs.
...
2. Share your public profile or a localized prefilled payment request URL.
3. Get paid directly to your wallet in Celo stablecoins with a shareable, reopenable in-app receipt.
...
- Share reopenable PayLink receipt URLs with matching Celo explorer transaction links for the same transaction
...
Use that printed network URL for local QR code, public profile, or localized prefilled payment request URL tests on the phone.
If you test generated public profile or localized prefilled payment request URLs on the phone, set `NEXT_PUBLIC_APP_URL` to the printed network origin and restart the dev server.
...
For final reviewer notes, regenerate the localized prefilled payment request URL from the deployed dashboard instead of adapting this local preview URL.
...
When sharing walletless review notes, label both preview URLs as local preview URLs so reviewers can compare the payment page and shareable, reopenable receipt preview.
...
Share the full unshortened PayLink receipt URL and Celo explorer transaction link so reviewers can see the deployed origin and transaction path.
Open the copied PayLink receipt URL in a signed-out browser once before sharing so reviewers see the same receipt without wallet state.
After a live payment, refresh the public profile once so reviewers can see the same amount, token, and reference in the recent PayLink contract event.
...
- Reuse the same localized prefilled payment request URL when comparing MiniPay and browser-wallet fallback behavior so amount, token, and reference stay aligned.
...
- Public profile payment pages, success receipts, and recent payments are designed to work without a backend, server session, or indexer.
...
- `SECURITY.md` asks private reports to include app surface, wallet prompt, PayLink URL type, receipt, and transaction context without secrets or unrelated wallet details.
...
- `package.json` keywords highlight MiniPay public profiles, public profile URLs, PayLink localized payment request URLs, PayLink prefilled payment request URLs, Celo stablecoin payment request URLs, payment proof, Celo receipts, Celo explorer transaction links, shareable and reopenable receipt URLs, no-custody settlement, walletless payment previews, bilingual and localized payments, and MiniPay PWA discovery.
- `layout.tsx` publishes root language and direction attributes, social and keyword metadata for MiniPay request URLs, PayLink prefilled and localized request URLs, Celo stablecoin payment request URLs, payment proof, Celo explorer transaction links, shareable and reopenable receipt URLs, bilingual and localized payments, and MiniPay PWA discovery, plus mobile web app tags, referrer policy, and Talent App verification metadata.
