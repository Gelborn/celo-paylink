...
Keep the target network name in the notes tab so final submission wording matches the wallet and explorer screens.
...
Keep a notes tab ready to paste the live PayLink receipt URL and Celo explorer transaction link so the reviewer handoff is immediate after confirmation.
Pre-label the notes tab fields for the PayLink receipt URL and Celo explorer transaction link so copied proof URLs do not get mixed up during recording.
...
Label the QR code as a public profile or localized PayLink prefilled payment request, matching the final submission notes, so reviewers know what should open on the phone.
...
Keep wallet account and network switcher menus closed unless you need to explain the separate recipient and payer wallet setup.
...
Keep browser zoom at 100% in walletless preview, live payment, receipt, and explorer tabs so the wallet handoff, receipt, and Celo explorer transaction page stay readable.

If the live wallet confirmation stalls during review, switch to the walletless preview URL and narrate where the live wallet confirmation would appear.
...
If the Celo explorer transaction page is slow during review, keep the PayLink receipt page visible and point out that the transaction link uses the same transaction hash.
...
3. Show the saved handle and copy the final public profile URL.
...
7. Send the payment and wait for confirmation.
   Keep the wallet confirmation visible briefly so the amount and token stay legible on camera.
...
8. Show the success receipt page and matching Celo explorer transaction link.
   ...
   Keep the Celo explorer transaction page visible long enough for reviewers to read the transaction hash and status.
   Copy the live PayLink receipt URL from the receipt page address bar after the live wallet walkthrough for the final submission notes.
   ...
   Keep the reloaded live receipt visible long enough to show the amount, token, reference, handle, and profile return path still match.
   Paste the copied live PayLink receipt URL into a fresh private browser tab once before submitting so reviewers see the same proof without payer wallet state.
