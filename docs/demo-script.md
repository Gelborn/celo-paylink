...
After any browser-wallet fallback network switch, re-check the amount, token, reference, and recipient handle on PayLink before approving.
...
1. Open the landing page and explain PayLink as a no-custody MiniPay payment profile in one sentence.
