...
Keep setup under 30 seconds so the live wallet confirmation, PayLink receipt page, and Celo explorer transaction page stay readable.
...
Keep the same final HTTPS localized PayLink prefilled payment request URL open in a second tab in case the payer MiniPay phone needs a quick reload.
Keep a notes tab ready for the live PayLink receipt URL and Celo explorer transaction link so the reviewer handoff is immediate after confirmation.
...
If you use a browser-wallet fallback instead of MiniPay, keep the same localized PayLink prefilled payment request URL so the amount, token, and reference stay aligned with the MiniPay path.
After any browser-wallet fallback network switch, re-check the amount, token, and reference on PayLink before approving.
...
8. Show the success receipt page and matching Celo explorer transaction link.
   ...
   Reopen the copied PayLink receipt URL once so the final proof is not tied to the payer wallet session.
   Keep the reloaded receipt visible long enough to show the amount, token, reference, and handle still match.
   Paste the copied PayLink receipt URL into a fresh private tab once before submitting so reviewers see the same proof without payer wallet state.
   Label the copied PayLink receipt URL separately from the Celo explorer transaction link in the final submission notes.
9. Refresh the public profile after the success receipt page and show the recent payment from PayLink contract events on Celo.
