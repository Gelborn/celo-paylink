...
Confirm the payer MiniPay wallet is on the target Celo network before opening the live payment request link.
...
Keep the public profile, payment request link, and PayLink receipt URLs visible long enough for reviewers to read the shared origin, handle, and receipt path.
...
If the live wallet confirmation stalls during judging, switch to the preview link and narrate where the live confirmation would happen.
