...
## Slide 1
- MiniPay PayLink
- A no-custody stablecoin payment profile for freelancers, creators, and solo merchants
...
- Low transaction costs for low-value repeat payments
...
- Share the PayLink receipt URL and matching Celo explorer transaction link as payment proof
- Reopen the PayLink receipt URL in a private tab to show it does not depend on the payer wallet session
- Refresh the public profile to show the matching recent PayLink contract event on that profile with the same amount, token, and reference
