...
- Rebuild after changing `NEXT_PUBLIC_APP_URL` so metadata, copied links, and receipt URLs use the final origin.
...
- Install the PWA once and confirm the `Open PayLink dashboard` shortcut shows the `My PayLink` short label, describes public profile URL, localized prefilled payment request links, recent incoming payments, and reopening PayLink receipt URLs, and opens `/my`.
...
- Confirm the final submission notes label the home URL, public profile URL, localized prefilled payment request link, PayLink receipt URL, and Celo explorer transaction link separately.
...
- Confirm the final submission notes name the target Celo network for the public profile, localized prefilled payment request link, PayLink receipt, and Celo explorer transaction link.
- Open each final submission URL in a signed-out or private browser before sending it to reviewers.
- Confirm the final localized prefilled payment request link uses a short reference that stays readable in MiniPay and on the receipt.
...
- After switching language, confirm the selected token label still matches the localized prefilled payment request link.
...
- Confirm profile creation, profile updates, and localized prefilled payment request link generation.
...
- Share one localized prefilled payment request link on mobile and confirm amount, token, and reference stay in the shared URL.
...
- Confirm the success-screen amount, token, and reference match the localized prefilled payment request link.
- Confirm the final notes use the same token symbol spelling shown in the wallet confirmation and PayLink receipt.
- Confirm the final localized prefilled payment request link and PayLink receipt URL use the same public profile handle.
