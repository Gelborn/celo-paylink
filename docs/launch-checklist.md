...
- Confirm `CELOSCAN_API_KEY` is set in the shell before running explorer verification scripts.
...
- Open the deployed home URL in a signed-out or private browser window and confirm it loads without a wallet connection.
...
- Install the PWA once and confirm the `Open PayLink dashboard` shortcut shows the `My PayLink` short label, describes profile management, payment request links, recent payments, and reopening receipt URLs, and opens `/my`.
...
- Open each final submission URL in a signed-out or private browser before sending it to reviewers and confirm no owner, recipient, or payer wallet session is required.
...
- At 200% browser zoom, confirm token, amount, reference, and submit controls remain visible without requiring horizontal scrolling.
...
- Test a payment request amount such as `5.50` and confirm the payment page and receipt keep the decimal value readable.
...
- Try a two-character profile handle once and confirm the dashboard keeps the profile unpublished with clear 3-character minimum handle guidance.
...
- Save the live demo transaction hash and target network with the final submission notes after the successful live payment.
- Save the matching PayLink receipt URL and Celo explorer transaction link beside the transaction hash and target network so reviewers can reopen the same proof links.
...
- Confirm the live receipt metadata description lists the amount, token, reference, recipient handle, matching Celo explorer transaction link, and profile return path.
...
- Confirm the uploaded demo description names the same target Celo network used by the PayLink receipt and Celo explorer proof, such as Celo mainnet or Celo Sepolia, before publishing or copying the submission URL.
- Label the uploaded demo URL as the demo video in the final submission notes, separate from the PayLink receipt URL and Celo explorer transaction link, so reviewers can identify each proof artifact.
- Open the copied uploaded demo URL in a private or signed-out browser once and confirm it is viewable without sign-in, password, or owner-only access.
- Confirm the uploaded demo URL remains public, avoids temporary or expiring share links, and stays available through the full review period.
