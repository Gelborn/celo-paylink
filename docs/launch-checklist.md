...
- Open one Brazilian Portuguese PayLink receipt page on mobile and confirm the amount, reference, and receipt actions remain readable.
...
- Reopen the PayLink receipt URL in a private browser window and confirm the amount, token, reference, and handle remain visible.
- Open the same PayLink receipt URL on mobile once and confirm the receipt stays readable.
- Use the browser back button from the success receipt page and confirm it returns to the public profile without an error.
...
- Capture the success receipt page only after the amount, token, reference, and explorer/profile actions are visible.
