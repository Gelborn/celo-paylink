...
- Confirm copied-link feedback is announced by a screen reader or exposed in the accessibility tree.
...
- Confirm the final notes use the same token symbol spelling shown in the wallet confirmation and PayLink receipt page.
...
- Reopen the PayLink receipt URL in a private or signed-out browser window and confirm the amount, token, reference, and handle remain visible without payer wallet state.
