...
- Share one localized PayLink prefilled payment request URL through the mobile share sheet and confirm amount, token, and reference stay encoded in the shared URL.
...
- Open the same live PayLink receipt URL on a mobile viewport at 100% zoom once and confirm the receipt details stay readable without horizontal scrolling.
...
- Confirm final payment and receipt screenshots show the selected token label next to every visible amount.
...
- Capture the success receipt page only after the amount, token, reference, PayLink receipt URL, and Celo explorer/profile actions are visible.
