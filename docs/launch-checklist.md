...
- Rebuild after changing `NEXT_PUBLIC_APP_URL` so metadata, copied URLs, and receipt URLs use the final origin.
...
- Open the `npm run dev:mobile` network host on the phone once before generating local QR codes or localized prefilled payment request URLs.
...
- Open the copied localized prefilled payment request URL on a second device and confirm the amount, token, and reference are prefilled.
...
- After switching language, confirm the selected token label still matches the localized prefilled payment request URL.
...
- Confirm profile creation, profile updates, and localized prefilled payment request URL generation.
...
- Share one localized prefilled payment request URL on mobile and confirm amount, token, and reference stay in the shared URL.
...
- Confirm the recent payment amount, token, and reference match the saved PayLink receipt.
...
- Confirm the success-screen amount, token, and reference match the localized prefilled payment request URL.
- Confirm the final notes use the same token symbol spelling shown in the wallet confirmation and PayLink receipt.
- Confirm the final localized prefilled payment request URL and PayLink receipt URL use the same public profile handle.
