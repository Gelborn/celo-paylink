...
- Open both manifest screenshot URLs and confirm the English and Brazilian Portuguese prefilled payment page and receipt proof preview posters load from the deployed origin with distinct locale labels.
- Install the PWA once and confirm the `Open PayLink dashboard` shortcut shows the `My PayLink` short label, describes public profile URL, payment request links, incoming payment, and reopening shareable receipts, and opens `/my`.
